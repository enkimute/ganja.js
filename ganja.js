...
//                   var p2 = Element.sw(e.motor,p);
    ...
    // It can be called with a function, a string, or used as a template function.
      ...
      // Our tokenizer reads the text token by token and stores it in the tok array (as type/token tuples).
        ...
        while (txt.length) for(t in tokens) if (resi = txt.match(tokens[t])) { var len = resi[0].length; if (len == 0) { var charCodes = []; for (var i=0; i<txt.length; i++) { var charCode = txt.charCodeAt(i); if (charCode > 255) charCodes.push(charCode); else break; } var codeString = String.fromCharCode(...charCodes); tok.push([t | 0, codeString]); txt = txt.slice(charCodes.length); } else { tok.push([t | 0, resi[0]]); txt = txt.slice(resi[0].length); } break; } // tokenise
