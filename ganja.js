  ...
  // Generate a new class for our algebra.
    ...
    // Parse expressions, translate functions and render graphs.
      ...
      static inline(intxt) {
        ...
        function translate(tokens) { var resi=[];
            for (var i=0,t; i<tokens.length; i++) // recurse brackets and unary operators into subarrays first.
              ...
              else if ((t=tokens[i])[1] == '[') { var open=1,sub=[],pre=[]; while(resi.length && (resi[resi.length-1][0]==5 || resi[resi.length-1][1]=='.')) pre.unshift(resi.pop()[1]);  while (open) { t = tokens[++i]; if (t[1] == '[') open++; else if (t[1] == ']') open--; if (open) sub.push(t); }; resi.push([[2,pre.join('')+'[']].concat(translate(sub)).concat([[2,']']])); }
              ...
              else if (t[1]=='-'&&resi.length&&(resi[resi.length-1][0]==4 || resi[resi.length-1][1]=='return'|| (resi[resi.length-1][0]==0 && resi[resi.length-2][1]=='return'))) { resi.push([[2,'']].concat(translate([t,tokens[++i]])))  }
              else resi.push(t);  
