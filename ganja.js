...
(function (name, context, definition) {
  ...
  // Generate a new class for our algebra.
    ...
    // Static operators (easier integration with scalars when using inline operator overloading)
      ...
      static Mul(a,b,res)   {  a=a.call?a():a; b=b.call?b():b; if ((a instanceof Array)&&(b instanceof Array)) { if(!(a[0] instanceof Array)&&!(b[0] instanceof Array)) { var r=tot?Element.Scalar(0):0; a.forEach((x,i)=>r=Element.Add(r,Element.Mul(x,b[i]))); return r; } else if (!(b[0] instanceof Array)) return b.map((x,i)=>Element.Mul(a[i],b)); var r=a.map((x,i)=>b[0].map((y,j)=>{ var r=tot?Element.scalar(0):0; x.forEach((xa,k)=>r=Element.Add(r,Element.Mul(xa,b[k][j]))); return r; })); return r;  } 
